...
```bash
# Find stanobj script (plugin or local skill)
STANOBJ_SCRIPT=$(find ~/.claude/plugins/cache -path "*/stanobj/skills/stanobj/scripts/stanobj.py" -type f 2>/dev/null | head -1)
[ -z "$STANOBJ_SCRIPT" ] && STANOBJ_SCRIPT="$HOME/.claude/skills/stanobj/scripts/stanobj.py"
python "$STANOBJ_SCRIPT" <input> -o <output.h5ad>
```
